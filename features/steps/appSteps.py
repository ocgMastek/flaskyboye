...
def fetch_patients_from_API(context):
    context.patients = requests.get("").json() #insert api get all patients
    pass
...
def check_count_increase(context):
    ok_((context.currentCount + 1) == len(requests.get("").json())) #insert api to get all patients
...
def delete_patient_with_API(context):
    context.countBeforeDelete = len(requests.get("").json()) #insert api to get all patients
    for row in context.table:
...
def check_count_decrease(context):
    ok_((context.countBeforeDelete - 1) == len(requests.get("").json())) #insert api to get all patients
